...
/* ============================================================
   LANDKREIS-FILTER – Kategorien & Barcode-Slot
   ============================================================ */
function applyDistrictFilter(){
  const isWittmund = settings.district === 'Landkreis Wittmund';

  /* Kategorien ein-/ausblenden */
  document.querySelectorAll('#catGrid .cbtn').forEach(btn=>{
    const k=btn.dataset.k;
    if(CATS_WITTMUND_ONLY.includes(k))
      btn.style.display = isWittmund ? '' : 'none';
    if(CATS_NICHT_WITTMUND.includes(k))
      btn.style.display = isWittmund ? 'none' : '';
  });

  /* Aktive Auswahl aufheben falls Kategorie jetzt ausgeblendet ist */
  if(cur && cur.category){
    const aktBtn=document.querySelector(`#catGrid .cbtn[data-k="${CSS.escape(cur.category)}"]`);
    if(aktBtn && aktBtn.style.display==='none'){
      cur.category=null;
      document.querySelectorAll('#catGrid .cbtn').forEach(b=>b.classList.remove('sel'));
    }
  }

  /* Barcode-Slot (slot2) und Erkennungsbox ein-/ausblenden */
  const slot2=document.getElementById('slot2');
  if(slot2){
    slot2.parentElement.style.display = isWittmund ? '' : 'none';
    if(!isWittmund){
      document.getElementById('bcBox').classList.add('hidden');
      if(cur) cur.barcode=null;
      const bcVal=document.getElementById('bcVal');
      if(bcVal) bcVal.textContent='';
    }
  }
}
